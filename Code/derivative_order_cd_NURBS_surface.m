function [DS] = derivative_order_cd_NURBS_surface(u,v,U,V,P,n,m,c,d)
    ...
    %% order (0,0) derivative (original function)
    aux = surface_points(n,m,P,u,v,U,V);
    H = H_Perspective(aux);
    DS(1,1,1:3) = H;
    if(c==1 & d==1)
        return;
    ...
    %- Computes the partial derivative general expression
    for k=1:c
        for l=1:d
            left_sum = 0;
            for i=2:k
